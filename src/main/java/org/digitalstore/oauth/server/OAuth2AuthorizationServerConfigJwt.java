...
public class OAuth2AuthorizationServerConfigJwt extends AuthorizationServerConfigurerAdapter implements OAuth2Constants
{
	...
	@Value("${sdx.oauth.token.expiry.time}")
	private Integer acessTokenValidity;
	
	@Value("${sdx.refresh.token.expiry.time}")
	...
	public void configure(final ClientDetailsServiceConfigurer clients) throws Exception
	{
		clients.inMemory()//
				...
				.accessTokenValiditySeconds(acessTokenValidity)
	...
	{
		if (OAuth2UserDetails.class.isInstance(successEvent.getAuthentication().getPrincipal()))
		{
			OAuth2UserDetails oauthUser = (OAuth2UserDetails) successEvent.getAuthentication().getPrincipal();
	...
	@EventListener
	public void onInternalSeverException(org.springframework.security.authentication.InternalAuthenticationServiceException exc) {
		exc.getStackTrace();
	}
	
	@EventListener
	public void onAuthenticationFailure(AuthenticationFailureBadCredentialsEvent failureEvent) {
		UserDTO user = userDao.findByUserName((String) failureEvent.getAuthentication().getPrincipal());
		if (user != null) {
			int loginCount = user.getInvalidLoginCount() == null ? 0 : user.getInvalidLoginCount();
			loginCount++;
			user.setInvalidLoginCount(loginCount);
			userDao.save(user);
		}
	}
